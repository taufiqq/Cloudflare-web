// File: functions/api/admin/token.js (Kredensial Dimasukkan Langsung)

// Fungsi publish MQTT dengan kredensial yang benar dan di-hardcode.
async function publishMqttViaApi(topic, payload) {
    ...
    // Kredensial dari 'curl' yang berhasil, dimasukkan langsung ke sini.
    const credentials = `z1107c66:A4BDT.XFU3GuxqbL`; 
    ...
    try {
        ...
        if (!response.ok) {
            console.error(`Gagal publish MQTT via API: Status ${response.status}`, await response.text());
        } else {
...
export async function onRequest(context) {
    ...
    // --- MENANGANI REQUEST (Tidak berubah) ---
    try {
            ...
            switch (action) {
                ...
                case 'generate_new': {
                    ...
                    if (oldData) {
                        ...
                        // Panggil fungsi publish (sudah ada await)
